    ...
    <table id="orders_table" class="table table-striped table-bordered">
        ...
        <tbody>
            ...
            ?>
                                            ...
                                            <tbody>
                                <?php
                                    // $get_orders = mysqli_query($db, "SELECT user_id, order_number, SUM(quantity) as quantity, SUM(price) as price, mop, s_fee, s_address, original_address, status FROM user_orders GROUP BY u_id, order_number");

                                    $get_orders = mysqli_query($db, "SELECT u_id, order_number, SUM(quantity) as quantity, SUM(price) as price, mop, s_fee, s_address, original_address, status FROM user_orders GROUP BY u_id, order_number");
                                    if ($get_orders) {
                                        while ($rows = mysqli_fetch_array($get_orders)) {
                                            $fullname = $rows['f_name'] . " " . $rows["l_name"];
                                ?>
